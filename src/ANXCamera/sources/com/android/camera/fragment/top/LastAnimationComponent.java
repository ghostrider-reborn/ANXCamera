...
public class LastAnimationComponent {
    ...
    public boolean reverse(boolean z) {
        ...
        this.mShown = false;
        if (this.mHidedViews != null) {
            for (View next : this.mHidedViews) {
                if (z) {
                    float f = 1.0f;
                    if (this.mAlphaMap.containsKey(next)) {
                        f = this.mAlphaMap.get(next).floatValue();
                    }
                    ViewCompat.setAlpha(next, 0.0f);
                    ViewCompat.animate(next).setStartDelay(0).setDuration(280).alpha(f).translationX(0.0f).setInterpolator(this.mQuartEaseOut).start();
