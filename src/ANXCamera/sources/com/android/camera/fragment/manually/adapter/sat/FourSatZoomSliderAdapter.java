...
public class FourSatZoomSliderAdapter extends AbstractZoomSliderAdapter {
    ...
    private void drawText(float f, int i, Canvas canvas) {
        canvas.save();
        canvas.translate(f, ((float) (this.mEntryLayouts[i].getLineAscent(0) - this.mEntryLayouts[i].getLineDescent(0))) / 2.0f);
    ...
    private static float[] getEntryX() {
        ...
        for (int i2 = 0; i2 < fArr.length; i2++) {
            if (fArr[i2] <= f) {
                ...
                fArr2[i2] = (((fArr[i2] - f) / ((float) (i - 1))) * 11.0f) + f;
            }
        }
        return fArr2;
    }
    ...
    private static float[] getZoomRatioY(float f) {
        ...
        for (int i2 = 0; i2 < fArr.length; i2++) {
            if (fArr[i2] <= 10.0f) {
                ...
                fArr2[i2] = (((fArr[i2] - 10.0f) / (((float) i) - 10.0f)) * (f - 10.0f)) + 10.0f;
            }
        }
        return fArr2;
    }
    ...
    public void draw(float f, int i, Canvas canvas, boolean z) {
        if ((i == 0 && !this.mIsLayoutRTL) || (i == ENTRY_INDEX_MAX && this.mIsLayoutRTL)) {
            drawText(f, indexToSection(i), canvas);
            f += measureWidth(i) - ((float) this.mLineWidth);
        }
        if (z) {
            canvas.drawLine(f, (float) (-this.mSelectHalfHeight), f, (float) this.mSelectHalfHeight, this.mSelectPaint);
        } else if ((this.mZoomRatioMax <= 10.0f || !(i == 0 || i == 4 || i == 14 || i == 24 || i == ENTRY_INDEX_MAX)) && (this.mZoomRatioMax > 10.0f || !(i == 0 || i == 10 || i == 30))) {
            canvas.drawLine(f, -this.mLineHalfHeight, f, this.mLineHalfHeight, this.mPaint);
        } else {
            canvas.drawLine(f, (float) (-this.mTargetHalfHeight), f, (float) this.mTargetHalfHeight, this.mPaint);
        }
        if ((i == ENTRY_INDEX_MAX && !this.mIsLayoutRTL) || (i == 0 && this.mIsLayoutRTL)) {
            drawText(f + ((float) this.mLineWidth) + ((float) this.mLineTextGap), indexToSection(i), canvas);
        }
    }
    ...
    public float mapPositionToZoomRatio(float f) {
        if (this.mZoomRatioMax > 10.0f) {
            return (0.0f > f || f >= 4.0f) ? (4.0f > f || f >= 14.0f) ? (14.0f > f || f > 24.0f) ? this.mRange10XToMaxEntryToZoomRatioSpline.interpolate(f) : this.mRange5XTo10XEntryToZoomRatioSpline.interpolate(f) : this.mRange1XTo5XEntryToZoomRatioSpline.interpolate(f) : this.mRangeMinTo1XEntryToZoomRatioSpline.interpolate(f);
        }
        if (0.0f <= f && f < 10.0f) {
            return this.mRange1XTo5XFor10XEntryToZoomRatioSpline.interpolate(f);
        }
        if (10.0f > f || f > 30.0f) {
            return 0.0f;
        }
        return this.mRange5XTo10XFor10XEntryToZoomRatioSpline.interpolate(f);
    }

    public float mapZoomRatioToPosition(float f) {
        if (this.mZoomRatioMax > 10.0f) {
            return (0.6f > f || f >= 1.0f) ? (1.0f > f || f >= 5.0f) ? (5.0f > f || f > 10.0f) ? this.mRange10XToMaxZoomRatioToEntrySpline.interpolate(f) : this.mRange5XTo10XZoomRatioToEntrySpline.interpolate(f) : this.mRange1XTo5XZoomRatioToEntrySpline.interpolate(f) : this.mRangeMinTo1XZoomRatioToEntrySpline.interpolate(f);
        }
        if (1.0f <= f && f < 5.0f) {
            return this.mRange1XTo5XFor10XZoomRatioToEntrySpline.interpolate(f);
        }
        if (5.0f > f || f > 10.0f) {
            return 0.0f;
        }
        return this.mRange5XTo10XFor10XZoomRatioToEntrySpline.interpolate(f);
    }
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:16:0x003e, code lost:
        if (r12.mCurrentValueIndex != r14) goto L_0x00d1;
     */
                ...
                if (i == -1) {
                    ...
                    if (!z2) {
                        ...
                        if (this.mManuallyListener != null) {
                            this.mManuallyListener.onManuallyDataChanged(this.mComponentData, this.mCurrentValue, str, false, this.mCurrentMode);
                        }
