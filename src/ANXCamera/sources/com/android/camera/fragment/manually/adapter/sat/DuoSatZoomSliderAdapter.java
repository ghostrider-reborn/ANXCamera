...
public class DuoSatZoomSliderAdapter extends AbstractZoomSliderAdapter {
    ...
    public DuoSatZoomSliderAdapter(Context context, ComponentData componentData, int i, ManuallyListener manuallyListener) {
        ...
        for (int i2 = 0; i2 < this.mEntries.length; i2++) {
            ...
            StaticLayout staticLayout = new StaticLayout(this.mEntries[i2], this.mTextPaint, Util.sWindowWidth, Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
    ...
    public float mapPositionToZoomRatio(float f) {
        return (0.0f > f || f > 10.0f) ? this.mEntryToZoomRatioSpline.interpolate(f) : this.mRange1XTo2XEntryToZoomRatioSpline.interpolate(f);
    }

    public float mapZoomRatioToPosition(float f) {
        return (1.0f > f || f > 2.0f) ? this.mZoomRatioToEntrySpline.interpolate(f) : this.mRange1XTo2XZoomRatioToEntrySpline.interpolate(f);
    }
    ...
    public void onPositionSelect(View view, int i, float f) {
        if (this.mEnable) {
            String valueOf = i <= 10 ? String.valueOf(HybridZoomingSystem.add(1.0f, ((float) i) * 0.1f)) : String.valueOf(mapPositionToZoomRatio(f * ((float) (getCount() - 1))));
            if (!valueOf.equals(this.mCurrentValue)) {
                ...
                if (this.mManuallyListener != null) {
                    this.mManuallyListener.onManuallyDataChanged(this.mComponentData, this.mCurrentValue, valueOf, false, this.mCurrentMode);
                }
