...
public class PagerGridLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    private Rect getItemFrameByPosition(int i) {
        ...
        if (rect == null) {
            ...
            if (canScrollHorizontally()) {
                i2 = 0 + (getUsableWidth() * i3);
            } else {
                i4 = 0 + (getUsableHeight() * i3);
                i2 = 0;
            }
            int i5 = i % this.mOnePageSize;
            int i6 = i5 / this.mColumns;
            int i7 = i2 + ((i5 - (this.mColumns * i6)) * this.mItemWidth);
            int i8 = i4 + (i6 * this.mItemHeight);
            rect.left = i7;
            rect.top = i8;
    ...
    private int getPageIndexByOffset() {
        if (canScrollVertically()) {
            ...
            int i = this.mOffsetY / usableHeight;
            return this.mOffsetY % usableHeight > usableHeight / 2 ? i + 1 : i;
        }
        int usableWidth = getUsableWidth();
        if (this.mOffsetX <= 0 || usableWidth <= 0) {
            return 0;
        }
        int i2 = this.mOffsetX / usableWidth;
        return this.mOffsetX % usableWidth > usableWidth / 2 ? i2 + 1 : i2;
    }
    ...
    private void recycleAndFillItems(RecyclerView.Recycler recycler, RecyclerView.State state, boolean z) {
        if (!state.isPreLayout()) {
            ...
            int pageIndexByOffset = (getPageIndexByOffset() * this.mOnePageSize) - (this.mOnePageSize * 2);
            if (pageIndexByOffset < 0) {
                pageIndexByOffset = 0;
            }
            int i = (this.mOnePageSize * 4) + pageIndexByOffset;
            if (i > getItemCount()) {
                i = getItemCount();
            }
            detachAndScrapAttachedViews(recycler);
            if (z) {
                while (pageIndexByOffset < i) {
                    addOrRemove(recycler, rect, pageIndexByOffset);
                    pageIndexByOffset++;
                }
                return;
            }
            for (int i2 = i - 1; i2 >= pageIndexByOffset; i2--) {
