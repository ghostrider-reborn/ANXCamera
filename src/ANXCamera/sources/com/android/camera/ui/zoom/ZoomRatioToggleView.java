...
public class ZoomRatioToggleView extends ViewGroup implements View.OnClickListener, View.OnLongClickListener {
    ...
    public ZoomRatioToggleView(@NonNull Context context, @Nullable AttributeSet attributeSet, int i, int i2) {
        ...
        this.mInactiveTask = new Runnable() {
            public void run() {
                ZoomRatioToggleView.access$300(ZoomRatioToggleView.TAG, "mInactiveTask: run...");
    ...
    private void endTouch(float f2) {
        int i;
        int i2;
        if (Math.abs(f2) >= 100.0f && !this.mIsImmersive && !this.mIsSuppressed && this.mSwipeAllowed) {
            int i3 = f2 > 0.0f ? 1 : -1;
            if (isLayoutRTL()) {
                if (i3 == 1) {
                    ...
                    i2 = this.mCurrentSelectedChildIndex;
                }
            } else if (i3 == 1) {
                i2 = this.mCurrentSelectedChildIndex;
            } else {
    ...
    public boolean isAnimating() {
        return this.mMovingAnimatorSet.isRunning() || this.mFadeInAnimatorSet.isRunning() || this.mFadeOutAnimatorSet.isRunning();
    }
    ...
    public void onDatasetChange(int i, boolean z) {
        ...
        if (length > 0) {
            if (!DataRepository.dataItemFeature().mb() || !z) {
                this.mMacroInSATDotSelected = false;
            } else {
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        if (action != 0) {
            ...
            if (toggleStateListener3 != null && !toggleStateListener3.isInteractive()) {
                return false;
            }
            startInactiveTimer();
            startTouch(motionEvent);
