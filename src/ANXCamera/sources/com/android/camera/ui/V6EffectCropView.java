...
public class V6EffectCropView extends View implements V6FunctionUI {
    ...
    private void computeCertenLineCrossPoints(Point point, Point point2) {
        ...
        if (squareOfPoints(point, point2) >= MIN_DIS_FOR_SLOPE) {
            ...
            if (point.x == point2.x) {
                ...
                int i5 = (int) (((float) point.y) - (((float) point.x) * f));
                if (i5 >= 0 && i5 <= height && !isContained(pointArr, 0, i5)) {
                    pointArr[i] = new Point(0, i5);
                    i++;
                }
                int i6 = (int) (((float) point.y) + (((float) (width - point.x)) * f));
                if (i6 >= 0 && i6 <= height && !isContained(pointArr, width, i6)) {
                    pointArr[i] = new Point(width, i6);
                    i++;
                }
                if (i == 1) {
                    i2 = i + 1;
    ...
    private void detectMovingEdges(float f, float f2) {
        ...
        if (this.mIsRect) {
            ...
            float f4 = (((float) CIRCLE_RESIZE_TOUCH_TOLERANCE) + width) * (width + ((float) CIRCLE_RESIZE_TOUCH_TOLERANCE));
            float f5 = f - centerX;
            float f6 = f2 - centerY;
                ...
                public void dispatchMessage(Message message) {
                    ...
                    switch (message.what) {
                        case 1:
                            ...
                            int unused = V6EffectCropView.this.mRangeWidth = V6EffectCropView.this.mAnimationStartRange + ((int) (((float) V6EffectCropView.this.mAnimateRangeWidth) * f));
                        ...
                        case 2:
                            ...
                            int unused2 = V6EffectCropView.this.mRadius = V6EffectCropView.this.mAnimationStartRadius + ((int) (((float) V6EffectCropView.this.mAnimateRadius) * f));
