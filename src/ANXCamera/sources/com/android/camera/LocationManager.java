...
public class LocationManager {
    ...
    private void startReceivingLocationUpdates() {
        ...
        if (this.mLocationManager != null) {
            try {
                ...
                Log.d(TAG, "provider does not exist " + e2.getMessage());
            }
            try {
    ...
    public Location getCurrentLocation() {
        ...
        for (LocationListener current : this.mLocationListeners) {
            Location current2 = current.current();
            if (current2 != null) {
                Log.v(TAG, "get current location, it is from " + this.mLocationListeners[r0].mProvider);
                return validateLocation(current2);
            }
        }
        StringBuilder sb = new StringBuilder();
