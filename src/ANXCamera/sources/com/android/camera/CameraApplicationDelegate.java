    ...
    public void onCreate() {
        ...
        GlobalConstant.sCameraSetupScheduler.scheduleDirect(new Runnable() {
            public void run() {
                ...
                if (!WatermarkMiSysUtils.isFileExist(Util.WATERMARK_FILE_NAME)) {
                    Util.generateWatermark2File();
                }
                if (DataRepository.dataItemFeature().Oc() && !WatermarkMiSysUtils.isFileExist(Util.WATERMARK_FRONT_FILE_NAME)) {
                    Util.generateFrontWatermark2File();
                }
                if (!WatermarkMiSysUtils.isFileExist(Util.WATERMARK_ULTRA_PIXEL_FILE_NAME)) {
