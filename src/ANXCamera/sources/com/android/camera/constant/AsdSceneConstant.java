...
public class AsdSceneConstant {
    private static final float AEC_LUX_HEIGHT_LIGHT = ((float) DataRepository.dataItemFeature().gt());
    private static final float AEC_LUX_LAST_LIGHT = ((float) DataRepository.dataItemFeature().gs());
    ...
    public static int parseRtbSceneResult(CaptureResult captureResult, boolean z, boolean z2) {
        ...
        if (!z) {
            ...
            mFrameNumber = 0;
            return -1;
        } else if (!DataRepository.dataItemFeature().ge() || mFrameNumber >= 2) {
            float f = ModuleManager.isFunARModule() ? AEC_LUX_HEIGHT_LIGHT * 0.7f : AEC_LUX_HEIGHT_LIGHT;
