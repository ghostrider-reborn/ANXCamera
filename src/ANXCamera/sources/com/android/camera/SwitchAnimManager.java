...
public class SwitchAnimManager {
    ...
    private void drawCopiedTexture(GLCanvas gLCanvas, int i, int i2, RawTexture rawTexture, float f) {
        if (this.mMoveBack) {
            ...
            float f3 = 1.0f - (ZOOM_DELTA_PREVIEW * f);
            float f4 = ((float) this.mReviewDrawingWidth) * f3;
            float f5 = ((float) this.mReviewDrawingHeight) * f3;
            rawTexture.draw(gLCanvas, Math.round(f2 - (f4 / 2.0f)), Math.round((((float) this.mReviewDrawingY) + (((float) this.mReviewDrawingHeight) / 2.0f)) - (f5 / 2.0f)), Math.round(f4), Math.round(f5));
            return;
        }
        ...
        float f8 = 1.0f - (ZOOM_DELTA_PREVIEW * f);
        float f9 = width * f8;
        float f10 = height * f8;
