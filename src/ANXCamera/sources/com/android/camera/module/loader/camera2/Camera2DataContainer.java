            ...
            while (i < length) {
                ...
                try {
                    ...
                    if (DataRepository.dataItemFeature().isSupportMacroMode() && 22 == parseInt) {
                        i++;
                    } else if (!DataRepository.dataItemFeature().hW() || 23 != parseInt) {
        ...
        if (i == 0) {
            if (CameraSettings.isDualCameraEnable() && (CameraSettings.isSupportedOpticalZoom() || CameraSettings.isSupportedPortrait()) && !DataRepository.dataItemGlobal().isForceMainBackCamera()) {
                switch (i2) {
                    case 162:
                    case 169:
                        if (CameraSettings.isMacroModeEnabled(i2) && DataRepository.dataItemFeature().hH()) {
                                ...
                                if (!CameraSettings.isSuperEISEnabled(i2)) {
                                    if (!CameraSettings.isUltraWideConfigOpen(i2) || HybridZoomingSystem.IS_3_OR_MORE_SAT) {
                                        if (HybridZoomingSystem.IS_3_OR_MORE_SAT) {
                                            ...
                                            if (f >= 1.0f) {
                                                if (HybridZoomingSystem.IS_4_SAT && f >= 2.0f) {
                                                    i3 = getAuxCameraId();
                                                    break;
                                                }
                                            } else {
                    ...
                    case 167:
                        if (CameraSettings.isZoomByCameraSwitchingSupported()) {
                            ...
                            i3 = ComponentManuallyDualLens.LENS_WIDE.equals(cameraLensType) ? getMainBackCameraId() : ComponentManuallyDualLens.LENS_TELE.equals(cameraLensType) ? getAuxCameraId() : ComponentManuallyDualLens.LENS_ULTRA.equals(cameraLensType) ? getUltraWideCameraId() : "macro".equals(cameraLensType) ? getStandaloneMacroCameraId() : ComponentManuallyDualLens.LENS_STANDALONE.equals(cameraLensType) ? getUltraTeleCameraId() : i;
                            break;
                        }
                    case 171:
