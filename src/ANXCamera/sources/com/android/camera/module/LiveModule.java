...
public class LiveModule extends BaseModule implements FocusManager2.Listener, ModeProtocol.CameraAction, Camera2Proxy.CameraPreviewCallback, Camera2Proxy.FaceDetectionCallback, Camera2Proxy.FocusCallback, Camera2Proxy.PictureCallback {
    ...
    public void mimojiLightDetect(Integer num) {
        if (this.mMimojiAvatarEngine != null && this.mMimojiAvatarEngine.isOnCreateMimoji()) {
            ...
            } else if (this.mMainProtocol != null) {
                this.mMainProtocol.updateMimojiFaceDetectResultTip(false);
            }
        }
    }
            ...
            if (!isFrameAvailable() || getCameraState() != 1) {
                GlobalConstant.sCameraSetupScheduler.scheduleDirect(new Runnable() {
                    public final void run() {
                        LiveModule.this.mCamera2Device.setDeviceOrientation(LiveModule.this.mOrientation);
                    }
                });
                return;
            }
    ...
    public int getOperatingMode() {
        return this.mModuleIndex == 177 ? CameraCapabilities.SESSION_OPERATION_MODE_MIMOJI : isPreviewEisOn() ? 32772 : DataRepository.dataItemFeature().fX() ? CameraCapabilities.SESSION_OPERATION_MODE_MCTF : 0;
    }
    ...
    public boolean onShutterButtonLongClick() {
        if (!checkCallingState()) {
            ...
            recordState.onFailed();
            return false;
        } else if (isRecording() || this.mMimojiAvatarEngine == null || this.mMimojiAvatarEngine.isOnCreateMimoji()) {
            return false;
        } else {
            startVideoRecording();
            return true;
    ...
    public void startPreview() {
        if (isDeviceAlive()) {
            ...
            if (this.mModuleIndex == 174) {
                ...
                this.mLiveConfigChanges.initPreview(this.mPreviewSize.height, this.mPreviewSize.width, isFrontCamera(), i);
                this.mLiveVideoEditor.setRecordParameter(this.mPreviewSize.height, this.mPreviewSize.width, DataRepository.dataItemLive().getLiveStartOrientation());
