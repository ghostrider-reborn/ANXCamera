...
public class CameraCapabilities {
    ...
    private List<StreamConfiguration> getStreamConfigurations() {
        ...
        if (isSupportedQcfa() && isTagDefined2 && isUltraPixelPhotographyMode()) {
            addStreamConfigurationToList(arrayList, CameraCharacteristicsVendorTags.SCALER_AVAILABLE_SR_STREAM_CONFIGURATIONS);
            return arrayList;
        } else if (!isSupportedQcfa() || !isTagDefined3 || (!isQcfaMode() && ((isTagDefined || isSupportedAndroidScalerStream()) && !isUltraPixelPhotographyMode()))) {
            if (isTagDefined) {
    ...
    private boolean isSupportedAndroidScalerStream() {
        return (b.sM || b.sQ) && getFacing() == 1;
    }

    private boolean isUltraPixelPhotographyMode() {
        return this.mOperatingMode == 33011 || this.mOperatingMode == 36868 || this.mOperatingMode == 33013 || this.mOperatingMode == 36871;
    }
    ...
    public float getMiAlgoASDVersion() {
        ...
        Float f = (Float) VendorTagHelper.getValueQuietly(this.mCharacteristics, CameraCharacteristicsVendorTags.MI_ALGO_ASD_VERSION);
        if (f == null) {
            return 0.0f;
        }
        return f.floatValue();
    ...
    public float getViewAngle(boolean z) {
        ...
        if (fArr != null && fArr.length > 0) {
            ...
            if (sizeF != null) {
                ...
                if (height > 0.0f) {
                    ...
                    Log.d(str, String.format(locale, "%s view angle: %.2f, size = %s, focalLength = %.4f", objArr));
                    return degrees;
                }
            }
        }
        ...
        Log.e(str2, String.format(locale2, "failed to get %s view angle", objArr2));
    ...
    public boolean isSupportMiMovie() {
        return isTagDefined(CaptureRequestVendorTags.MiMOVIE_ENABLED.getName());
    }
    ...
    public boolean isSupportUltraPixelPortrait() {
