...
public interface AvatarConfig {
    ...
    public static class ASAvatarProcessInfo {
        ...
        public boolean checkFaceBlocking() {
            ...
            float f17 = f9 / 12.0f;
            float f18 = f10 / 12.0f;
            LOG.d("CheckOutLine", "leftFace = " + f13);
            LOG.d("CheckOutLine", "rightFace = " + f14);
            LOG.d("CheckOutLine", "leftEyeBrow = " + f15);
            LOG.d("CheckOutLine", "rightEyeBrow = " + f16);
            LOG.d("CheckOutLine", "leftEye = " + f17);
            LOG.d("CheckOutLine", "rightEye = " + f18);
            LOG.d("CheckOutLine", "nose = " + f);
            LOG.d("CheckOutLine", "mouth = " + f2);
            LOG.d("CheckOutLine", "chin = " + f3);
            if (f13 > 0.5f && f15 > 0.5f && f17 > 0.5f) {
                LOG.d("CheckOutLine", "--- > left is blocking <---");
                return true;
            } else if (f14 > 0.5f && f16 > 0.5f && f18 > 0.5f) {
                LOG.d("CheckOutLine", "--- > right is blocking <---");
                return true;
            } else if (f15 <= 0.4f || f16 <= 0.4f || f18 <= 0.4f || f17 <= 0.4f) {
                int i3 = (f3 > 0.4f ? 1 : (f3 == 0.4f ? 0 : -1));
                if (i3 > 0 && f2 > 0.4f && f > 0.4f) {
                    LOG.d("CheckOutLine", "--- > central is blocking <---");
                    return true;
                } else if (f13 <= 0.4f || f14 <= 0.4f || i3 <= 0) {
                    return false;
                } else {
        ...
        public int checkOutLineInfo() {
            float f;
            int i;
            float f2 = this.faceOrientations[0];
            float f3 = this.faceOrientations[1];
            int i2 = 2;
            float f4 = this.faceOrientations[2];
            ...
            LOG.d("CheckOutLine", "fFaceValue = " + f);
            if (f > OUTLINE_THRESHOLD_VALUE) {
                return 6;
            }
            float f7 = 0.0f;
