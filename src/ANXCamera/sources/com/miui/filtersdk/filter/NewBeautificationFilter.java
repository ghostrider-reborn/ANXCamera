...
public class NewBeautificationFilter extends BaseBeautyFilter {
    ...
    private static void dumpToBitmap(int i, int i2, ByteBuffer byteBuffer) {
        ...
        if (byteBuffer != null) {
            ...
            try {
                fileOutputStream = new FileOutputStream(new File(Environment.getExternalStorageDirectory().getPath() + "/hehe/" + System.currentTimeMillis() + Storage.JPEG_SUFFIX));
                try {
    ...
    public void passPreviewFrameToTexture(final byte[] bArr, final int i, final int i2) {
        if (!hasPendingDrawTasks()) {
            runOnDraw(new Runnable() {
                public void run() {
                    int unused = NewBeautificationFilter.this.mTextureId = NewBeautificationFilter.this.mBeautyProcessor.beautify(bArr, i, i2);
