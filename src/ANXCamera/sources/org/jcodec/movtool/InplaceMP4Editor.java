...
public class InplaceMP4Editor {
    ...
    public boolean copy(File file, File file2, MP4Edit mP4Edit) throws IOException {
        ...
        try {
            ...
            try {
                List<Tuple._2<MP4Util.Atom, ByteBuffer>> doTheFix = doTheFix(fileChannelWrapper2, mP4Edit);
                if (doTheFix == null) {
                    z = false;
                } else {
                    ...
                    for (MP4Util.Atom next : MP4Util.getRootAtoms(fileChannelWrapper2)) {
                        ByteBuffer byteBuffer = (ByteBuffer) asMap.get(Long.valueOf(next.getOffset()));
                        if (byteBuffer != null) {
                            fileChannelWrapper.write(byteBuffer);
                        } else {
                            next.copy(fileChannelWrapper2, fileChannelWrapper);
                        }
                    }
                    z = true;
                }
