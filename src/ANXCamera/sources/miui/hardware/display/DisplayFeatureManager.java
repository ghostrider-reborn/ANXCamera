...
public class DisplayFeatureManager {
    ...
    public void setScreenEffect(int mode, int value) {
        synchronized (this.mLock) {
            String str = TAG;
            Slog.d(str, "setScreenEffect mode=" + mode + " value=" + value);
            if (this.mProxy == null) {
                initProxyLocked();
            }
            if (this.mProxy != null) {
    ...
    public void setScreenEffect(int mode, int value, int cookie) {
        synchronized (this.mLock) {
            String str = TAG;
            Slog.d(str, "setScreenEffect mode=" + mode + " value=" + value + " cookie=" + cookie);
            if (this.mProxy == null) {
                initProxyLocked();
            }
            if (this.mProxy != null) {
    ...
    private void initProxyLocked() {
        try {
            ...
            IBinder b2 = ServiceManager.getService(DISPLAY_FEATURE_SERVICE);
            if (b2 != null) {
                b2.linkToDeath(this.mBinderDeathHandler, 0);
                this.mProxy = new DisplayFeatureServiceProxy(b2);
            }
